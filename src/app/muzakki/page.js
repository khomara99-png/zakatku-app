...
export default function MuzakkiPage() {
  ...
  const [form, setForm] = useState({ nama: '', alamat: '', no_hp: '', jumlah_jiwa: 1 })

  async function fetchData() {
    setLoading(true)
    const { data, error } = await supabase.from('muzakki').select('*').order('created_at', { ascending: false })
  ...
  function handleTambah() {
    setEditing(null)
    setForm({ nama: '', alamat: '', no_hp: '', jumlah_jiwa: 1 })
    setShowForm(true)
  }

  function handleEdit(item) {
    setEditing(item.id)
    setForm({ nama: item.nama || '', alamat: item.alamat || '', no_hp: item.no_hp || '', jumlah_jiwa: item.jumlah_jiwa || 1 })
    setShowForm(true)
  }
  ...
  async function handleSubmit(e) {
    ...
    if (editing) {
      const { error } = await supabase.from('muzakki').update(form).eq('id', editing)
      if (error) alert('Gagal update: ' + error.message)
    } else {
      const { error } = await supabase.from('muzakki').insert(form)
    ...
    <main className="max-w-5xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-500 text-sm">Daftar pemberi zakat ({list.length} orang)</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl shadow p-8 text-center text-gray-500">Belum ada data. Klik "+ Tambah Muzakki".</div>
      ) : (
      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
            <h2 className="text-xl font-bold mb-4 text-gray-800">{editing ? 'Edit Muzakki' : 'Tambah Muzakki'}</h2>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nama *</label>
                <input type="text" value={form.nama} onChange={(e) => setForm({ ...form, nama: e.target.value })} className={inputClass} placeholder="Contoh: Ahmad Fauzi" required />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Alamat</label>
                <input type="text" value={form.alamat} onChange={(e) => setForm({ ...form, alamat: e.target.value })} className={inputClass} placeholder="Contoh: Jl. Melati No. 5" />
              </div>
              <div>
