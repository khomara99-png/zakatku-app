  ...
  async function handleCetakMuzakki() {
    ...
    try {
      ...
      ;(jiwaData || []).forEach((j) => {
        ...
        if (!muzId) return
        if (!muzakkiIdsPeriode.has(muzId)) return
      ...
      console.log('Muzakki list count:', muzakkiList.length)
    ...
    <main className="max-w-4xl mx-auto p-6">
      ...
      <div className="bg-white rounded-xl shadow p-5 mb-6">
        ...
        <p className="text-xs text-slate-500 mt-3">
          Data difilter berdasarkan tanggal pendaftaran (created_at). Kosongkan
          tanggal untuk melihat semua data.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="bg-white rounded-xl shadow p-5">
              ...
              >
                {proses
                  ? 'Memproses...'
                  : `🖨️ Cetak Laporan Muzakki (${muzakkiList.length})`}
              </button>
            </div>
            ...
            <div className="bg-white rounded-xl shadow p-5">
              ...
              >
                {proses
                  ? 'Memproses...'
                  : `🖨️ Cetak Laporan Mustahik (${mustahikList.length})`}
              </button>
            </div>
          </div>
        </>
